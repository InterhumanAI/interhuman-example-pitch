...
function getProjectRef(supabaseUrl) {
  ...
  if (!match) {
    throw new Error(
      "NEXT_PUBLIC_SUPABASE_URL must look like https://your-project.supabase.co"
    );
  }
  return match[1];
}
...
async function main() {
  ...
  if (!databaseUrl && (!supabaseUrl || !password)) {
    throw new Error(
      "Set SUPABASE_DB_URL or both NEXT_PUBLIC_SUPABASE_URL and SUPABASE_DB_PASSWORD before running db:setup."
    );
  }
